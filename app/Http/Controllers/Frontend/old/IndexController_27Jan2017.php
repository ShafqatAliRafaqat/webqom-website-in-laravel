...
class IndexController extends Controller
{
    ...
    public function index()
    {
        ...
        $offer_services = OfferSerivce::where('status','1')->get();
        ...
        /*foreach ($categories as $i) {
            echo '<pre>'.$i->title;
            if($i->sub_categories){
                foreach ($i->sub_categories as $childs) {
                    # code...
                echo "-childs".count($i->sub_categories);
                }
            }
            echo '</pre>';
        }*/
         
        return view("frontend.index",compact('indexplans','offer_services','page','page_cms','videos','testimonial','general_features'));
    }
    public static function get_categories()
    {
        ...
        foreach ($categories as $i) {
            ...
            if($i->category_images){
                ...
                where('status',1)->
                where('image_end_date','>=',date("Y-m-d"))->
                sortBy('sort_order');
            }
        }
        return $categories;
