...
<div class="clearfix margin_bottom5"></div>
    ...
    @foreach($plans as $i)
		...
		@if(!empty($service_free_domains))
			<?php //echo '<pre>'; print_r($featured_plans); exit; ?>
            <ul>
            @foreach($service_free_domains as $ij)
			<?php
				$Type = $ij->type;
				$Fees = '';
				if($Type==1){
					...
					$TypeText = "Offer a discounted domain registration/transfer only (renew as normal), with FEE "; $Fees = $ij->fee;
				}elseif($Type==4){
