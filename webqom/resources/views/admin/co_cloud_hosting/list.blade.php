...
array('url'=>'javascript:void','name'=>'Cloud Hosting- Edit'),
];
?>
@extends('layouts.admin_layout')
@section('title','Admin | Cloud Hosting- Edit')
@section('content')
...
<div class="page-content">
      ...
      <div id="hosting-plans" class="tab-pane fade in active">
        ...
        <div class="portlet">
          <div class="portlet-header">
            ...
            <p style="margin-top: 32px" class="margin-top-10px"></p>
            <a href="{{url('admin/cloud_hosting/new')}}" class="btn btn-success">Add New Plan &nbsp;<i class="fa fa-plus"></i></a>&nbsp;
            <div class="btn-group">
              <button type="button" class="btn btn-primary">Delete</button>
              <button type="button" data-toggle="dropdown" class="btn btn-red dropdown-toggle"><span class="caret"></span><span class="sr-only">Toggle Dropdown</span></button>
              <ul role="menu" class="dropdown-menu">
              ...
              <table class="table table-hover table-striped">
                ...
                <tbody>
                  <form id="delete_hosting_plans">
                    {!!csrf_field()!!}
                    @if(!empty($cloud_hosting_plan))
                    @foreach($cloud_hosting_plan as $i)
                    <tr>
                      ...
                      <td>@if($i->price_type=='Recurring') <span>RM</span> {{$i->price_annually or '0'}}<span>/yr</span>@endif
                        ...
                        <td><a href="{{url('/admin/cloud_hosting/edit/'.$i->id)}}"  title="Edit"><span class="label label-sm label-success"><i class="fa fa-pencil"></i></span></a> <a href="#" data-hover="tooltip" data-placement="top" title="Delete" data-target="#modal-delete-plan-feature-{{$i->id}}" data-toggle="modal"><span class="label label-sm label-red"><i class="fa fa-trash-o"></i></span></a>
                          <!--Modal Edit plan feature start-->
                      ...
                      <div class="form">
                       <form id="frm_general_features" class="form-horizontal" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                          ...
                          <div class="col-md-6">
                            <input type="hidden" name="page" class="form-control" value="cloud hosting">
                  ...
                  <table class="table table-hover table-striped">
                    ...
                    <tbody>
                      <form id="delete_general_features">
                        ...
                        <?php $count_gf++; ?>
                        <tr>
                          <td><input type="checkbox" value="{{$i->id}}" name="id_gf[]"></td>
                          <td>{{$count_gf}}</td>
                          <td><span class="label label-xs label-{{$i->status?'success':'danger'}}">{{$i->status?'Active':'Inactive'}}</span></td>
                          <td>{{$i->title}}</td>
          ...
          <div class="form">
            ...
            <div class="form-group">
              ...
              <div class="col-md-6">
                ...
                <input type="hidden" name="page" type="hidden" class="form-control" value="cloud hosting">
                <input type="text" name="title"   class="form-control" placeholder="eg. Ultra-Fast Platform ">
                <p class="red_error" id="gf_title"></p>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="form-actions">
                <div class="col-md-offset-5 col-md-8"> <button type="button" id="frm_general_features_edit_submit" class="btn btn-red">Save &nbsp;<i class="fa fa-floppy-o"></i></button>&nbsp; <a href="#" data-dismiss="modal" class="btn btn-green">Cancel &nbsp;<i class="glyphicon glyphicon-ban-circle"></i></a> </div>
              </div>
                </form>    
  ...
  <script type="text/javascript">
    page_url=base_url+'/admin/general_features/';  
    ...
    $(document).on('submit', '#plan_feature_frm', function(event) {
      ...
      $.ajax({
        url: base_url+'/admin/cloud_hosting/new',
        type: 'POST',
    ...
    $(document).on('click', '#edit_plan_feature', function(event) {
      $.ajax({
        url: base_url+'/admin/cloud_hosting/update',
        type: 'POST',
    ...
    function save_content(publish) {
      ...
      .done(function(response) {
        ...
        if (publish==0) {
          window.open(base_url+"/admin/cloud_hosting/preview");
        }
   ...
   $(document).on('click', '#update_so', function(event) {
     var data=[];
     data='<?php echo json_encode($cloud_hosting_plan) ?>';
    ...
    $.ajax({
     url: base_url+'/admin/cloud_hosting/update_sort',
     type: 'POST',
