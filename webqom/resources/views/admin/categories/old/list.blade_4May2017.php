...
<div class="page-content">
							...
							<li data-id="{{$i->id}}" class="dd-item">
								...
								<div class="dd-handle"> {{$i->title}} 
									...
									<div class="pull-right dd-nodrag">
										...
										<a href="#" data-hover="tooltip" data-placement="top"  data-target="#modal-edit-{{$i->id}}" data-toggle="modal" title="Edit"><span class="label label-sm label-success"><i class="fa fa-pencil"></i></span></a>
										<!-- <a href="{{url('admin/index-plan')}}" data-hover="tooltip" data-placement="top" title="Add Plan &amp; Page Edit"><span class="label label-sm label-warning"><i class="fa fa-plus"></i></span></a> -->
										...
										<li data-id="{{$j->id}}" class="dd-item">
											<div class="dd-handle">{{$j->title}} 
												...
												<div class="pull-right dd-nodrag">
													...
													<a href="javascript:void" data-hover="tooltip" data-placement="top"  data-target="#modal-edit-{{$j->id}}" data-toggle="modal" title="Edit"><span class="label label-sm label-success"><i class="fa fa-pencil"></i></span></a>
			...
			<div class="modal-dialog modal-wide-width " id="modal-upload-image-body">
				<div class="modal-content">
					...
					<div id="msg_error" class="alert alert-danger alert-dismissable">
						...
						<div class="form">
							<form class="form-horizontal" id="upload_image_form">
								{{csrf_field()}}
	...
	<div id="upload_content_section" hidden="">
								...
								<label class="col-md-3 control-label">Status1</label>
									<div class="col-md-6">
										<div data-on="success" data-off="primary" class="make-switch" onclick="changemyclass()">
											<input name="upload_status[]" type="checkbox" checked="checked"/>
										</div>
									</div>
								</div>  
	...
	function upload_category_images(category_id) {
		//debugger;
			...
			$.ajax({
				...
				//var urls = base_url+'/admin/categories/reload_switcher/';
				//alert(response);
				  
				/*$.post( base_url+"/admin/categories/reload_switcher", {_token:"{{ csrf_token() }}"}).done(function(data){
					$('.make-switch').bootstrapSwitch();
					$("#makesw_response").html(data); 
				});*/ 
				 $.each(response, function(k, v) {
					$('#upload_section').append('<section id=s'+v.id+'></section>');  
					$('#s'+v.id).append($('#upload_container_1').html()); 
					//alert($('#upload_container_1').html());
					/*$.post( base_url+"/admin/categories/reload_switcher", {_token:"{{ csrf_token() }}"}).done(function(data){
						$('#s'+v.id).append(data); 
					});*/
				...
				$.each(response, function(k, v) { 
					...
					//alert(myDateFormatter(v.image_start_date));
					$('#s'+v.id).find($('.upload_start_date')).val(myDateFormatter(v.image_start_date));
					$('#s'+v.id).find($('.upload_end_date')).val(myDateFormatter(v.image_end_date));
