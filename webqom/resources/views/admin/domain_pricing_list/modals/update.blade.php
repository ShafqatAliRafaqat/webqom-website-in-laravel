	...
	<div class="modal-dialog modal-wide-width">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="form">
					<form class="form-horizontal" method="post" action="{{ route('domain_pricing_list.update', $i->id) }}">
						...
						<div class="form-group">
							...
							<div class="col-md-6">
								<div class="xss-margin"></div>
								<div class="checkbox-list">
									<label><input type="checkbox" data-toggler data-toggle=".toggle-update-{{$type}}" {{ $i->addons === 'DNS Managment;Email Forwarding;ID Protection' ? 'checked' : '' }}>&nbsp; Select all</label>
									<label><input class="toggle-update-{{$type}}" name="addons[]" type="checkbox" value="DNS Managment" <?=in_array('DNS Managment', $addons) ? 'checked' : ''?>>&nbsp; DNS Management</label>
									<label><input class="toggle-update-{{$type}}" name="addons[]" type="checkbox" value="Email Forwarding" <?=in_array('Email Forwarding', $addons) ? 'checked' : ''?>>&nbsp; Email Forwarding</label>
									<label><input class="toggle-update-{{$type}}" name="addons[]" type="checkbox" value="ID Protection" <?=in_array('ID Protection', $addons) ? 'checked' : ''?>>&nbsp; ID Protection</label>
								</div>
							</div>
						</div>
