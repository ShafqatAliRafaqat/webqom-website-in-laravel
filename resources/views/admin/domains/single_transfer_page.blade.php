        ...
        <div class="col-lg-12">
            ...
            @if(Session::has('delete-success'))
                <div class="alert alert-success alert-dismissable">
                    <button type="button" data-dismiss="alert" aria-hidden="true" class="close">&times;</button>
                    <i class="fa fa-check-circle"></i> <strong>Success!</strong>
                    <p>{{ Session::get('delete-success') }}</p>
                </div>
            @endif
        ...
        <div class="col-lg-12">
            ...
            <div class="portlet">
                <div class="portlet-header">
                    ...
                    <!--Modal Add new general feature start-->
                                    ...
                                    <div class="form">
                                        <form action="{{route('add_feature_single_transfer_page')}}" class="form-horizontal" method="POST">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-6">
                                                    <input name="title" required type="text" class="form-control" placeholder="eg. Competitive Price">
                                                </div>
                                            </div>

                                            <div class="form-group">
                                                <label class="col-md-3 control-label">Feature Description <span class="text-red">*</span></label>
                                                <div class="col-md-6">
                                                    <textarea name="description" class="form-control" required ></textarea>
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label class="col-md-3 control-label">Insert CSS Icon or <span class="text-red">*</span></label>
                                                <div class="col-md-6">
                                                    ...
                                                    <input name="icon" type="text" required  class="form-control" id="inputContent" placeholder="eg. fa-rocket or icon-anchor">
                            ...
                            <div class="modal-content">
                                ...
                                <div>
                                    <form action="{{route('delete_feature_single_transfer_page')}}" method="POST">
                                        {{csrf_field()}}
                                        <div class="modal-body" id="delete-items-modal-body">
                                            <div class="form-actions">
                                                <div class="col-md-offset-4 col-md-8"> <button type="submit" class="btn btn-red">Yes &nbsp;<i class="fa fa-check"></i></button>&nbsp; <a href="#" data-dismiss="modal" class="btn btn-green">No &nbsp;<i class="fa fa-times-circle"></i></a> </div>
                                            </div>
                                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-striped">
                            ...
                            @foreach((array)json_decode($transferPage->featureList) as $feature)
                                <tr>
                                    <td><input type="checkbox" id="selectFeature" class="selectFeature" feature-title="{{$feature->title}}" feature-id="{{$loop->index}}"></td>
                                    <td>{{$loop->index+1}}</td>
                                    <td><span class="label label-xs label-success">Active</span></td>
                                    <td>{{$feature->title}}</td>
    ...
    <script type="text/javascript">
        $('.save-page').click((e)=>{
            ...
            $.ajax({
                ...
                window.scrollTo(0,0); //added by Narek
            })
        })
        ...
        $('#check-all').click(function(event) {
            if(this.checked) {
                ...
                console.log($('input#selectFeature'));
                $('input#selectFeature').each(function() {
                    this.checked = true;
                });
            } else {
                $('input#selectFeature').each(function() {
        ...
        //Added by mrloffel, modified by Narek
        function deleteSelected(){
            let items = $('input#selectFeature:checked[type=checkbox]')
            if(items.length != 0) {
                $('#delete-items-modal-body').html('<div id="list-of-selected"></div>' +
                    '<div class="form-actions">' +
                ...
                var checkboxTitles = $('input#selectFeature:checked[type=checkbox]').map(function(i, val){
                    return ':</strong> ' + $(this).attr("feature-title") + '</p>';
                });
                var checkboxIDs = $('input#selectFeature:checked[type=checkbox]').map(function(i, val){
                    return Number($(this).attr("feature-id"));
                });

                $('#list-of-selected > p').remove();
                var titles = [];
                for(var i = checkboxTitles.size(); i > 0; --i){
                    titles[i-1] = checkboxIDs[i-1];
                    $('#list-of-selected').prepend('<p><strong>#' + checkboxIDs[i-1] + checkboxTitles[i-1]);
                }
                titles = JSON.stringify(titles);
                $('#list-of-selected > input[name=selected]').remove();
                $('#list-of-selected').prepend('<input name="selected" type="hidden" value="' + titles + '">')
            } else {
                $('#delete-items-modal-body').html('<div class="alert alert-danger alert-dismissable">' +
                    '<i class="fa fa-times-circle"></i> <strong>Error!</strong>' +
